...
- Runtime modes: title, gameplay, dev. `dev` remains an internal compatibility mode during migration; the title screen exposes only the normal game path. The legacy dev topic dock has been removed, and `resource-debug` is the only valid topic-panel target.
